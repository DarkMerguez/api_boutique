...
app.post("/user/login", checkJWT, async (request, response)=>{
    ...
    const payload = {name : user.name, role : user.getRole()};
    const newToken = jwt.sign(payload, secret);

    const valid = await bcrypt.compare(loginBody.password, user.password);
});
